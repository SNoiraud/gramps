...
from gramps.gen.filters.rules.citation import (
    RegExpIdOf,
    HasCitation,
    HasTag,
    HasNoteRegexp,
...
class CitationSidebarFilter(SidebarFilter):
    ...
    def create_widget(self):
        ...
        self.add_entry(
            _("Min. Conf.", "Citation: Minimum Confidence"), self.filter_conf
        )
    ...
    def get_filter(self):
        ...
        if empty:
            ...
            rule = HasSource(
                [src_title, src_author, src_abbr, src_pub],
                use_regex=regex,
                use_case=usecase,
            )
            generic_filter.add_rule(rule)
            ...
            if src_note:
                rule = HasSourceNoteRegexp(
                    [src_note], use_regex=regex, use_case=usecase
                )
                generic_filter.add_rule(rule)
